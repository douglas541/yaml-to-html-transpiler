...
const string basePath = @"C:\Users\dougl\Documents\Dev\C#\HtmlCompiler\";
...
static void AppendImage(HtmlNode node, int lineIndex, string[] lines, int identationLevel, Match? imageMatch = null)
{
	...
	for (var imageLineIndex = lineIndex + 1; imageLineIndex < lines.Length; imageLineIndex++)
	{
		...
		if (pathMatch.Success)
		{
			...
			var httpMatch = Regex.Match(imgPath, @"^(http(s)?://)|^(www\.)");
				        
			if (!httpMatch.Success && (imgPath.StartsWith("/") || imgPath.StartsWith(@"\")))
			{
				imgPath = imgPath.Substring(1, imgPath.Length - 1).Replace(@"\", "/");
			}
			else
			{
...
File.WriteAllText(@"C:\Users\dougl\Documents\Dev\C#\HtmlCompiler\Files\output.html", htmlDoc.DocumentNode.OuterHtml);